...
def no_email(any_dictionary, any_key):
    string = ''
    for i, j in any_dictionary.items():
        if any_key not in j or j.get(any_key) == '':
            string += j.get('first name') + ' '
    return string


print(f'{no_email(dictionary, key)}don`t have an email')
# key = 'email'
